...
spots_FF_filt %>% group_by(gene,day, image, cell) %>% mutate(day = ifelse(day == "S8", "fasting", "refed"), gene = ifelse(gene == "FF", "Firefly", ifelse(gene == "Lox", "NTC", gene))) %>% summarise(num_spots = n()) %>% unite(col = "sample",gene,day,sep = "_") %>% ggplot(aes(x = sample, y = num_spots, fill = sample, alpha = sample)) + geom_boxplot(width = 0.25, outlier.shape = NA) + geom_point(alpha = 0.01, position = "jitter") + theme_cowplot() + guides(fill = FALSE, alpha = FALSE) + labs(x = "", y = "Number of spots per cell") + EnvStats::stat_n_text(y.pos = -1.5) + theme(axis.text.x=element_text(angle=60,hjust=1,vjust=1)) + scale_fill_manual(values = c("#808080","#808080","#808080","#808080", "#e8c040", "#e8c040","#e8c040", "#e8c040", "#b82080", "#b82080","#b82080", "#b82080")) + scale_alpha_manual(values = c(0.5,1,0.5,1,0.5,1,0.5,1,0.5,1,0.5,1)) 

spots_FF_filt %>% mutate(Position_In_Z = (-(as.numeric(Pos_Z)) + 10750)/1000) %>% filter(gene != "Lox") %>% mutate(gene = ifelse(gene == "FF", "Firefly", gene)) %>% ggplot(aes(x = day, y = Position_In_Z))  + geom_violin(aes(fill = gene, alpha = day)) + geom_boxplot(aes(fill = gene, alpha = day),width = 0.1, outlier.shape = NA) + stat_compare_means(comparisons = list(c("S8", "S8RF2")), method = "wilcox.test", label.y = 7) + theme_cowplot() + facet_grid(.~gene) + scale_fill_manual(values = c("#808080", "#e8c040", "#e8c040", "#b82080", "#b82080")) + scale_alpha_manual(values = c(0.5,1)) + guides(fill = FALSE, alpha = FALSE) + coord_cartesian(ylim = c(-0,8)) + labs(x ="", y = "Position in Z (um)") + scale_x_discrete(labels = c("fasting", "refed")) +
...
spots_S28TC_filt <- spots_S28TC %>% filter(TH_fit == 1, as.numeric(INT_raw) > 2200, as.numeric(BGD) < 1800, as.numeric(Pos_Z) > 750)
