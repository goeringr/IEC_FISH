...
spots_FF_filt %>% mutate(Position_In_Z = (-(as.numeric(Pos_Z)) + 9000)/1000) %>% filter(gene != "Lox") %>% mutate(gene = ifelse(gene == "FF", "FIREFLY", gene)) %>% ggplot(aes(x = day, y = Position_In_Z)) + geom_point(position = "jitter", alpha = 0.1) + geom_violin(aes(fill = gene, alpha = day)) + geom_boxplot(aes(fill = gene, alpha = day),width = 0.1,outlier.shape = NA) + stat_compare_means(comparisons = list(c("S8", "S8RF2")), method = "wilcox.test", label.y = 6) + theme_cowplot() + facet_grid(.~gene) + scale_fill_manual(values = c("#808080", "#e8c040", "#e8c040", "#b82080", "#b82080")) + scale_alpha_manual(values = c(0.5,1)) + geom_hline(yintercept = 0) +guides(fill = FALSE, alpha = FALSE) + coord_cartesian(ylim = c(-0.1,6.5)) + labs(x ="", y = "Position in Z (um)") + scale_x_discrete(labels = c("fasting", "refed")) +
