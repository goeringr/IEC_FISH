...
spots_7d_filt %>% group_by(gene, image, cell) %>% summarise(num_spots = n()) %>% filter(gene != "Sdc2") %>% ggplot(aes(x = gene, y = num_spots, fill = gene)) + geom_violin() + geom_boxplot(width = 0.25) + geom_point(alpha = 0.01, position = "jitter") + theme_cowplot() + guides(fill = FALSE, col = FALSE) + stat_compare_means(comparisons = list(c("Lox", "FF"),c("Lox", "Col12a1"), c("Lox", "Osbpl3"), c("Lox", "SerpinF2")), method = "wilcox.test", label.y = c(100,80,120,140)) + labs(x = "", y = "Number of spots per cell") + scale_fill_manual(values = c("#c551ee", "#808080", "#808080", "#9840e3", "#66f2e0")) + scale_x_discrete(limits = c("FF", "Lox", "Col12a1", "Osbpl3", "SerpinF2"), labels = c("FIREFLY","NTC", "COL12A1", "OSBPL3", "SERPINF2")) + EnvStats::stat_n_text(y.pos = -20)

##### 0 -> 13000 nm ~ 13 microns?? uhhh...

spots_7d_filt %>% mutate(Position_In_Z = (-(as.numeric(Pos_Z)) + 15000)/1000) %>% filter(gene == "FF") %>% summarize(median(Position_In_Z)) 

spots_7d_filt %>% mutate(Position_In_Z = (-(as.numeric(Pos_Z)) + 15000)/1000) %>% filter(gene != "Lox") %>% ggplot(aes(x = gene, y = Position_In_Z, fill = gene)) + geom_violin() + geom_boxplot(width = 0.25, outlier.shape = NA) + geom_point(alpha = 0.001, position = "jitter") + theme_cowplot() + guides(fill = FALSE, col = FALSE) + stat_compare_means(comparisons = list(c("FF", "Col12a1"), c("FF", "Osbpl3"), c("FF", "Sdc2"), c("FF", "SerpinF2")), method = "wilcox.test") + scale_fill_manual(values = c("#c551ee", "#808080", "#9840e3", "#48c8db", "#66f2e0")) + labs(x = "", y = "Position in Z (um)") + geom_hline(yintercept = 4.58, size = 1, linetype = "dashed") + geom_hline(yintercept = 0) + scale_x_discrete(limits = c("FF", "Col12a1", "Osbpl3", "Sdc2", "SerpinF2"))

#Sdc2 color = , "#48c8db"

spots_7d_filt %>% mutate(Position_In_Z = (-(as.numeric(Pos_Z)) + 15000)/1000) %>% filter(gene != "Lox", gene != "Sdc2") %>% ggplot(aes(x = gene, y = Position_In_Z, fill = gene)) + geom_violin() + geom_boxplot(width = 0.25, outlier.shape = NA) + geom_point(alpha = 0.001, position = "jitter") + theme_cowplot() + guides(fill = FALSE, col = FALSE) + stat_compare_means(comparisons = list(c("FF", "Col12a1"), c("FF", "Osbpl3"),  c("FF", "SerpinF2")), method = "wilcox.test", label.y = c(14,15,16)) + scale_fill_manual(values = c("#c551ee", "#808080", "#9840e3", "#66f2e0")) + labs(x = "", y = "Position in Z (um)") + geom_hline(yintercept = 4.58, size = 1, linetype = "dashed") + geom_hline(yintercept = 0) + scale_x_discrete(limits = c("FF", "Col12a1", "Osbpl3", "SerpinF2"), labels = c("FIREFLY", "COL12A1", "OSBPL3", "SERPINF2")) + coord_cartesian(ylim = c(-1,17))

spots_7d_filt %>% mutate(Position_In_Z = (-(as.numeric(Pos_Z)) + 12000)/1000) %>% filter(gene != "Lox") %>% ggplot(aes(x = Position_In_Z, fill = gene)) + geom_density(alpha = 0.5) + theme_cowplot() + scale_fill_manual(values = c("#c551ee", "#808080", "#9840e3", "#48c8db", "#66f2e0")) + labs(x = "Position in Z (um)") 
...
spots_21d_filt %>% filter(gene != "Lox") %>% ggplot(aes(-(as.numeric(Pos_Z))+10000, col = gene)) + stat_ecdf(size = 1.5) + theme_cowplot() + xlim(0, 8000) + labs(x = "Position in Z", y = "Proportion of spots")
